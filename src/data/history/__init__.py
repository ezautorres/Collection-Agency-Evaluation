...
def build_agency_history(
    ...
    keys = ["client_id", "fidespid", "fitipodepto", "week"]
    ...
    df_sum = (
        ...
        .groupBy(*keys)
        .agg(
            F.sum("fnmontoabono").alias("fnmontoabono_efect"),
            F.sum("fncargoautomatico").alias("fncargoautomatico_efect"),
            F.sum("fnmontocargoautomatico").alias("fnmontocargoautomatico_efect"),
            F.sum("fnmontocaptacion").alias("fnmontocaptacion_efect"),
            F.count("*").alias("n_abonos_efect"),
        )
    )
    ...
    w = (
        Window
        .partitionBy(*keys)
        .orderBy(F.col("fdfechabono").desc())
    )
    ...
    df_last = (
        ...
        .select(
            *keys,
            F.col("fisematrasomax").alias("fisematrasomax_efect"),
            F.col("fidiasatraso").alias("fidiasatraso_efect"),
            F.col("ficlasifcuenta").alias("ficlasifcuenta_efect"),
            F.col("fidiasatrasoabono").alias("fidiasatrasoabono_efect"),
            F.col("fitipocartera").alias("fitipocartera_efect"),
            F.col("fiindjudextraju").alias("fiindjudextraju_efect"),
            F.col("fiorigencartera").alias("fiorigencartera_efect"),
            F.col("fnsaldocte").alias("fnsaldocte_efect"),
            F.col("fnsaldoatrasadocte").alias("fnsaldoatrasadocte_efect"),
            F.col("fnmoratorioscte").alias("fnmoratorioscte_efect"),
            F.col("fitelefonia").alias("fitelefonia_efect"),
        )
    )

    tacuabonado = df_sum.join(df_last, on=keys, how="inner")

    #recup = (
    #    spark.table("rd_baz_bdclientes.rd_recuperacion_cuotas_cobranza")
    #    .where(
    #        (F.col("fisemana") >= F.lit(start_week)) &
...
#
    ...
    #        ).alias("client_id"),
#
    #        F.col("fidespacho").alias("fidespid"),
    #        "fitipodepto",
    #        F.col("fisemana").alias("week"),
#
    #        F.greatest(
...
#
    ...
    df_final = (
        threphonorarios
        .join(tacuabonado, on=["client_id", "fidespid", "fitipodepto", "week"], how="left")
    )

    keys_week = ["fidespid", "fitipodepto", "week"]
    ...
    df_week = (
        ...
        .agg(
            ...
            F.sum("fnrecupcapital").alias("recupcapital"),
            F.sum("fnrecupmoratorios").alias("recupmoratorios"),
            F.sum("fnmontoabono_efect").alias("abono_efectivo"),

            F.avg("fidiasatraso").alias("dias_atraso_prom"),
            F.avg("fidiasatraso_efect").alias("dias_atraso_efect_prom"),

            F.avg("fiporcentajecomision").alias("comision_prom"),

            F.avg("fnsaldocte_efect").alias("saldo_prom"),

            F.sum("n_gestiones").alias("gestiones"),
            F.sum("n_abonos_efect").alias("abonos"),
        )
    )

    keys_final = ["fidespid", "fitipodepto"]
    ...
    df_desp = (
        ...
        .agg(
            # tamaño / volumen
            F.avg("n_clientes").alias("clientes_prom"),
            F.max("n_clientes").alias("clientes_max"),

            # cobranza
            F.avg("recupcapital").alias("recupcapital_prom"),
            F.sum("recupcapital").alias("recupcapital_total"),

            F.avg("abono_efectivo").alias("abono_efectivo_prom"),

            # eficiencia
            (F.sum("recupcapital") / F.sum("gestiones")).alias("recup_por_gestion"),

            # riesgo
            F.avg("dias_atraso_prom").alias("dias_atraso_promedio"),
            F.max("dias_atraso_prom").alias("peor_semana_atraso"),

            # actividad
            F.avg("gestiones").alias("gestiones_prom"),
            F.avg("abonos").alias("abonos_prom"),

            # estabilidad
            F.stddev("recupcapital").alias("recup_volatilidad"),

            # comisión
            F.avg("comision_prom").alias("comision_promedio"),
        )
    )
...
root
 |-- fianio: integer (nullable = true)
 |-- fisemana: integer (nullable = true)
 |-- fcfoliohonorario: string (nullable = true)
 |-- fitipofolio: integer (nullable = true)
 |-- fitipodepto: integer (nullable = true)
 |-- fncomision: decimal(15,4) (nullable = true)
 |-- fnporcentajecom: decimal(15,2) (nullable = true)
 |-- fngestionesinv: decimal(15,2) (nullable = true)
 |-- fnconcargoauto: decimal(15,2) (nullable = true)
 |-- fncobdespacho: decimal(15,2) (nullable = true)
 |-- fiestatusfolio: integer (nullable = true)
 |-- fcusuariostatus: string (nullable = true)
 |-- fdfechastatus: long (nullable = true)
 |-- fniva: decimal(15,4) (nullable = true)
 |-- fnsubtotal: decimal(15,4) (nullable = true)
 |-- fnretencioniva: decimal(15,4) (nullable = true)
 |-- fnretencionisr: decimal(15,4) (nullable = true)
 |-- fntotalapagar: decimal(15,4) (nullable = true)
 |-- fcfopresupuesto: string (nullable = true)
 |-- fcfoliopoliza: string (nullable = true)
 |-- fcfoelectronico: string (nullable = true)
 |-- fiiddespacho: long (nullable = true)
 |-- fdregistro: long (nullable = true)
 |-- ultima_modificacion: long (nullable = true)
 |-- usuario_modifico: string (nullable = true)
 |-- fcfoliofact: string (nullable = true)
 |-- fcfcdi: string (nullable = true)
 |-- fcfoliodoctrasc: string (nullable = true)
 |-- fcusuariocreacion: string (nullable = true)
 |-- fdfechacreacion: long (nullable = true)
...
root
 ...
 |-- fisemana: integer (nullable = true)
 |-- fitipodepto: integer (nullable = true)
 |-- fideptoid: integer (nullable = true)
 |-- fcempnum: string (nullable = true)
 |-- fidespid: long (nullable = true)
 |-- figestor: long (nullable = true)
 |-- fitipocartera: integer (nullable = true)
 |-- fiproducto: integer (nullable = true)
 |-- ficampania: integer (nullable = true)
 |-- fitotalclientes: long (nullable = true)
 |-- fnsaldo: decimal(15,2) (nullable = true)
 |-- fnmoratorios: decimal(15,2) (nullable = true)
 |-- fnsaldoatrasado: decimal(15,2) (nullable = true)
 |-- fncobranzaefectivo: decimal(15,2) (nullable = true)
 |-- fncobranzarmd: decimal(15,2) (nullable = true)
 |-- fncargosaut: decimal(15,2) (nullable = true)
 |-- fntotal: decimal(15,2) (nullable = true)
 |-- finumabonosefectivo: long (nullable = true)
 |-- finumcargosaut: long (nullable = true)
 |-- usuario_modifico: string (nullable = true)
 |-- ultima_modificacion: long (nullable = true)
 |-- fnmontocargoautomatico: decimal(15,2) (nullable = true)
 |-- fnmontocaptacion: decimal(15,2) (nullable = true)
 |-- fcusuariocreacion: string (nullable = true)
 |-- fdfechacreacion: long (nullable = true)
